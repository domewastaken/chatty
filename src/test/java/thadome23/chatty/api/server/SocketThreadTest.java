...
class SocketThreadTest extends Thread{
	private ChatServerTest	 delete;
	private Socket			 socket;
	private PrintStream 	 to_client;
	private BufferedReader 	 from_client;
	private Chat_roomTest 		 room;
	private String			 userName;

	SocketThreadTest(Socket s, ChatServerTest chatServerTest)
	{
		this.delete = chatServerTest ;
		this.socket = s;
						...
						if(text.startsWith("/"))
						{
							...
							switch (parameter[0]) {
								...
								case "/create":
									...
									if (room != null){
										room.deleteUser(SocketThreadTest.this);
									}
									try {
										room = new Chat_roomTest(parameter[1], 5);
									} catch (Exception e) {
										sendInfo("this name already exist.Try another one");
										isPresent = true;
									}
									if (!isPresent) {
										room.joinRoom(SocketThreadTest.this);
								...
								case "/join":
									...
									room = Chat_roomTest.getRoomByName(parameter[1]);

									if (room == null) {
										sendInfo("" + parameter[1] + " room doesn't exist.");
									} else if (!room.joinRoom(SocketThreadTest.this)) {
										sendInfo("" + parameter[1] + " room is full.");
									}else {sendRoomName(room.getName());}
								break;

								case "/show":
									sendInfo(getStringActiveRooms(Chat_roomTest.getAvailableRooms()));
								break;

								case "/switch":
									Chat_roomTest newRoom = Chat_roomTest.getRoomByName(parameter[1]);
									if (newRoom!=null){
										room.deleteUser(SocketThreadTest.this);
										room = newRoom;
										room.joinRoom(SocketThreadTest.this);
	...
	private String getStringActiveRooms(ArrayList<Chat_roomTest> c) {
		String str="";
		if (!c.isEmpty()){
			for (Chat_roomTest room: c) {
